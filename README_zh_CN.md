...
kew all            （随机播放音乐库中最多 20000 首歌曲）

kew albums         （随机依次播放最多 2000 张专辑）

kew moonlight son  （查找并播放《月光奏鸣曲》）

kew beet           （播放 “beethoven” 目录下的所有音乐）

kew dir <专辑名>    （当名称冲突时，可指定为目录）

kew song <歌曲名>   （播放指定歌曲）

kew list <播放列表名> （播放指定播放列表）

kew theme midnight  （设置 “midnight.theme” 主题）

kew shuffle <专辑名> （打乱播放顺序，shuffle 参数需放在最前）

kew artistA:artistB:artistC （随机播放这三位艺术家的作品）
...
导出播放列表, 按 x. 保存播放列表为 .m3u 文件，文件名为当前队列首首歌曲。
