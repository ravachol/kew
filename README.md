...
Install through your package manager or homebrew (macOS). If you can't find it on your distro, or you want the bleeding edge, follow the [Manual Installation Instructions](docs/MANUAL-INSTALL-INSTRUCTIONS.md). For NixOS, there is an official package but also a [Nix Flake](flake.nix) that is tied to the latest commit. For Artix it's available in the [artist repository](https://wiki.artixlinux.org/Main/Repositories#artist_repository). For Fedora there is an [unofficial repo](https://copr.fedorainfracloud.org/coprs/fed500/kew).
