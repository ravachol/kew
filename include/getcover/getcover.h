        ...
        if ((dp = readdir(dirp)) != NULL)
            ...
            case DT_REG: /* regular file */
                ...
                if ((fp = fopen(filepath, "r")) != NULL)
                {
                    ...
                    {
                        if (verbose)
                            (void)printf("WAV FILE: %s\n", dp->d_name);
                    }
                    else if (memcmp(str, "ID3\3", 4) == 0)
                    {
                        if (verbose)
                            (void)printf("mp3 FILE: %s\n", dp->d_name);
...
int extractID3v2_cover(const char *filePath, const char *outputFilePath)
{
    ...
    {
        if (verbose)
            printf("Cover image header not found.\n");
        fclose(file);
        return -1;
    }
    ...
    while (fread(header, sizeof(unsigned char), 2, file) == 2)
    {
        ...
        {
            if (verbose)
                printf("Found JPEG footer!\n");
            fclose(file);
            fclose(outputFile);
            if (verbose)
                printf("Cover image saved to: %s\n", outputFilePath);
            return 0;
        }
...
static int get_FLAC_cover(FILE *fp, const char *outputFilepath)
{
    ...
    do
    {
        ...
        case 6: /* Picture data */
            ...
            if (verbose)
                printf("  Number of colors for indexed-color = %d\n", number_of_indexed_color);
            fread(buf, 1, 4, fp);

            picture_length = ((buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3]);
            if (verbose)
                printf("  Length of picture data = %d\n", picture_length);
    ...
    while ((size = fread(coffset, 1, 4, fp)) > 0)
    {
        ...
        { /* look into moov box */
            ...
            do
            {
                ...
                boxtype[4] = 0;
                if (verbose)
                    printf(" box type = moov:%s\n", boxtype);
                if (memcmp(boxtype, "udta", 4) == 0)
                { /* look into udta box */
                    sum3 = 0;
                    do
                    {
                        ...
                        boxtype[4] = 0;
                        if (verbose)
                            printf(" box type = moov:udta:%s\n", boxtype);
                        if (memcmp(boxtype, "meta", 4) == 0)
                        { /* look into meta */
                            sum4 = 0;
                            do
                            {
                                ...
                                sum4 += offset4;
                                if (verbose)
                                    printf(" box type = moov:udta:meta:%s\n", boxtype);
                                if (memcmp(boxtype, "ilst", 4) == 0)
                                { /* look into ilst */
                                    do
                                    {
                                        sum5 = 0;
                                        ...
                                        sum5 += offset5;
                                        if (verbose)
                                            printf(" box type = moov:udta:meta:ilst:%s\n", boxtype);
                                        if (memcmp(boxtype, "covr", 4) == 0)
                                        { /* look into cover */
                                            size = fread(coffset, 1, 4, fp);
                                            ...
                                            boxtype[4] = 0;
                                            if (verbose)
                                                printf(" box type = moov:udta:meta:ilst:covr:%s\n", boxtype);
                                            ...
                                            {
                                                if (verbose)
                                                    printf("  File %s exists, did not override.\n", picture_path);
                                                /*if(!override) {
                                                    fclose(picture_fp);
                                                    return DONE;
                                                }*/
                    ...
                    fseek(fp, offset2 - 8, SEEK_CUR);
            } while (sum2 < offset1);
        } /* end of moov search */
        else
            fseek(fp, offset1 - 8, SEEK_CUR);
    }
    return NOTYET;
}
