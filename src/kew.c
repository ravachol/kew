...
void notifySongSwitch(SongData *currentSongData, UISettings *ui, AppState *state)
{
    if (currentSongData != NULL && currentSongData->hasErrors == 0 && currentSongData->metadata && strnlen(currentSongData->metadata->title, 10) > 0)
    {
        ...
        if (state->current_lyrics) {
            free_lyrics(state->current_lyrics);
        }
        state->current_lyrics = load_lyrics(currentSong->song.filePath);
    }
}
...
void determineSongAndNotify(UISettings *ui)
{
    ...
    if (currentSong && lastNotifiedId != currentSong->id)
    {
        if (!isDeleted)
            notifySongSwitch(currentSongData, ui, &appState);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    // Theme initialization based on the new system
    if (argc == 3 && strcmp(argv[1], "theme") == 0) {
        ...
        loadTheme(&appState, &settings, ui->themeName, false);
    } else {
